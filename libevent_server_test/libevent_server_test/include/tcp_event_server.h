//
// Created by robot on 4/12/22.
//

#ifndef TCP_EVENT_SERVER_H_
#define TCP_EVENT_SERVER_H_
...
public:
  ...
  __attribute__((unused)) int GetFd() const { return m_fd; }

  //获取可读数据的长度
  __attribute__((unused)) int GetReadBufferLen()
  { return (int)evbuffer_get_length(m_ReadBuf); }

  //从读缓冲区中取出len个字节的数据，存入buffer中，若不够，则读出所有数据
  //返回读出数据的字节数
  __attribute__((unused)) int GetReadBuffer(char *buffer, int len)
  { return evbuffer_remove(m_ReadBuf, buffer, len); }

  //从读缓冲区中复制出len个字节的数据，存入buffer中，若不够，则复制出所有数据
  //返回复制出数据的字节数
  //执行该操作后，数据还会留在缓冲区中，buffer中的数据只是原数据的副本
  __attribute__((unused)) int CopyReadBuffer(char *buffer, int len)
  { return (int)evbuffer_copyout(m_ReadBuf, buffer, len); }

  //获取可写数据的长度
  __attribute__((unused)) int GetWriteBufferLen()
  { return (int)evbuffer_get_length(m_WriteBuf); }

  //将数据加入写缓冲区，准备发送
  __attribute__((unused)) int AddToWriteBuffer(char *buffer, int len)
  { return evbuffer_add(m_WriteBuf, buffer, len); }

  //将读缓冲区中的数据移动到写缓冲区
  __attribute__((unused)) void MoveBufferData()
